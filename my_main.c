...
struct vary_node ** second_pass() {
  ...
  for(int i = 0; i < lastop; i++){
    if(op[i].opcode == VARY){
      double increment = (op[i].op.vary.end_val - op[i].op.vary.start_val) / (op[i].op.vary.end_frame - op[i].op.vary.start_frame);
      for (int frame = op[i].op.vary.start_frame; frame <= op[i].op.vary.end_frame; frame++){
	if (frame > num_frames){
	  break;
	}
	double value = op[i].op.vary.start_val + (frame - op[i].op.vary.start_frame) * increment;
